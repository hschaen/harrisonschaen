...
class AJAX_Content_Navigator {
	...
	/* Loop a group filter */
	function loop($filter, $type=null, $sortby='date') {
		$display = null;
		switch($filter) {
	...
	/* Browser/links/navi */
	function browser( $type=null, $options=null, $forcetaxonomy=null, $forcetaxonomies=null, $forceterm=null, $pricefilter=null, $sort=null, $sortby=null ) {
		...
		if ($acn_filters) {
			...
			foreach($acn_filters as $filter) {
				...
				$display .= '<h3>'.$this->get_group_name($filter).'</h3>
					<div><ul>'.$this->loop($filter, $type, $sortby).'</ul></div>';
